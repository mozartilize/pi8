...
2. **Uncertainty never cheapens routing.** Missing, failed, or low-confidence assessments keep at least the heuristic task type; unknown quality outranks measured weak quality. Entry downgrades follow `adoptAssessment` (high-confidence, bounded). Mid-intent `plan`/`review` → `implement` needs a high-confidence `implement` verdict and an identified mutation call (any scope); it takes effect next invocation unless a trajectory handoff is pending or the depth latch is engaged. Keep the served model's capability minimum and the incumbent thinking minimum. Scored effort may exceed its minimum, never fall below it; investigation discounts are bounded and economic promotion requires measured quality.
