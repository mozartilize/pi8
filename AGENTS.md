...
2. **Uncertainty never cheapens routing.** Missing, failed, or low-confidence assessments keep at least the heuristic task type; unknown quality outranks measured weak quality. Entry downgrades follow `adoptAssessment` (high-confidence, bounded). Mid-intent `plan`/`review` → `implement` happens only through an accepted `commit_execution` plan; a mutation call alone never changes the task type. The executor minimum comes from the submitter's rubric plus router measurements: an unscored criterion or a failed measurement counts as harder, and the plan's shape can only raise the band. Only a released plan may drop the incumbent capability and thinking minimums; everything else keeps both. A broken plan returns the next invocation to its submitter's model, task type, and thinking level; a plan another model executed returns to its submitter as `review` for the rest of the entry. An executor model with two strikes (breaks, or executed plans the submitter replaced) is replaced only by a strictly stronger model one band higher. Scored effort may exceed its minimum, never fall below it; investigation discounts are bounded and economic promotion requires measured quality.
