...
describe('assessment telemetry', () => {
  ...
  it('stamps the prompt version and mode when an assessment ran', () => {
    appendDecision(
      decision({
    ...
    expect(entry.assessmentMode).toBe('active');
...
describe('appendShadowAssessment', () => {
  let dir: string;
  ...
  it('writes a joinable counterfactual record without touching decisions', () => {
    ...
    const entry = JSON.parse(readFileSync(join(dir, DECISION_LOG_FILE), 'utf8').trim());
    expect(entry.kind).toBe('assessment-shadow');
  ...
  it('records latch-transition metadata on the shadow record', () => {
    appendShadowAssessment(
      {
        intentKey: 'k3',
        heuristicDimension: 'gather',
  ...
  it('records an unavailable shadow attempt with its fallbackReason', () => {
    appendShadowAssessment(
  ...
  it('never throws when the log path is unwritable', () => {
    ...
    expect(() =>
      appendShadowAssessment({ intentKey: 'k', heuristicDimension: 'gather' }, join(blocker, 'sub')),
    ).not.toThrow();
  });
});
