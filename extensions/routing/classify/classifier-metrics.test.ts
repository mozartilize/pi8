...
//   correct on all en rows and on 3/5 es rows;
//   es "explica la arquitectura" (gold gather) → plan (over-route by 3);
//   es "evalúa la calidad del código" (gold review) → gather (under-route by 2).
...
const CORPUS: LabeledPrompt[] = [
  ...
  { prompt: 'hola amigo', lang: 'es', goldDimension: 'lightweight' },
  { prompt: 'explica la arquitectura', lang: 'es', goldDimension: 'gather' },
  { prompt: 'escribe código para mí', lang: 'es', goldDimension: 'implement' },
  { prompt: 'diseña la arquitectura del sistema', lang: 'es', goldDimension: 'plan' },
  { prompt: 'evalúa la calidad del código', lang: 'es', goldDimension: 'review' },
];
...
const predictMap: Record<string, Dimension> = {
  ...
  'hola amigo': 'lightweight',
  'explica la arquitectura': 'plan', // wrong: over-route from gather
  'escribe código para mí': 'implement',
  'diseña la arquitectura del sistema': 'plan',
  'evalúa la calidad del código': 'gather', // wrong: under-route from review
};
...
describe('classifier metrics', () => {
  ...
  it('per-language macro-F1 matches hand-computed one-vs-rest math', () => {
    ...
    const es = evaluateLanguage(CORPUS, 'es', predict);
    // es per-dimension F1: lightweight 1, gather 0, plan 0.666..., implement 1, review 0.
    expect(es.perDimension.lightweight.f1).toBe(1);
    expect(es.perDimension.gather.f1).toBe(0);
    expect(es.perDimension.plan.f1).toBeCloseTo(2 / 3, 6);
    expect(es.perDimension.implement.f1).toBe(1);
    expect(es.perDimension.review.f1).toBe(0);
    expect(es.macroF1).toBeCloseTo((1 + 0 + 2 / 3 + 1 + 0) / 5, 6);
    expect(es.agreement).toBe(3 / 5);
    // costs: row "explica" over-route 3 steps × OVER(1) = 3; row "evalúa"
    // under-route 2 steps × UNDER(2) = 4; total 7 / 5 rows.
    expect(es.costWeightedError).toBeCloseTo(7 / 5, 6);
  });

  it('overall macro-F1 averages the per-language macro-F1s', () => {
    const en = evaluateLanguage(CORPUS, 'en', predict).macroF1;
    const es = evaluateLanguage(CORPUS, 'es', predict).macroF1;
    expect(macroF1(CORPUS, predict)).toBeCloseTo((en + es) / 2, 6);
  ...
  it('cost-weighted error encodes under-route as costlier than over-route', () => {
    const under = CORPUS.filter((row) => row.prompt === 'evalúa la calidad del código');
    const over = CORPUS.filter((row) => row.prompt === 'explica la arquitectura');
  ...
  it('compareClassifiers runs every predictor over the same corpus apples-to-apples', () => {
    ...
    for (const name of ['perfect', 'flawed']) {
      expect(Object.keys(reports[name].perLanguage).sort()).toEqual(['en', 'es']);
    }
